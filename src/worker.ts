/// <reference path="../tsd/bencode.d.ts" />

import dgram = require('dgram');
import crypto = require('crypto');
import util = require('util');
import bencode = require('bencode');
import remoteNodes = require('./redis/remoteNodes');
import bucket = require('./redis/bucket');
...
class Worker {
    ...
    run() {
        ...
        remoteNodes.pop(function (error, reply) {
            ...
            // 不停的发送
            setTimeout(this.run.bind(self), config.worker[self.port].cycle);
        });
    }
     ...
     * socket的消息处理
     */
    onmessage(packet: Buffer, rinfo: { address: string, port: number }): void | boolean {
        var self = this;
     ...
     * 响应错误
     */
    error(target: { address: string, port: number }, msg, code, text: string): void {
        // 不给本机发送请求
        if (this.ips.indexOf(target.address) !== -1) {
            return;
        }
